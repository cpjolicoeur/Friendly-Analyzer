...
function startLoadingTracker(total = 0) {
  ...
  loadingTimerId = setInterval(() => {
    ...
    const countText = loadingState.total ? `${loadingState.completed}/${loadingState.total} complete` : "Starting up";
...
async function loadMatches() {
  ...
  try {
    ...
    console.log(`[${clubDisplayName}] Found squad ID: ${squadId}`);
    ...
    const matchReports = await mapWithConcurrency(matchesData, MAX_CONCURRENT_MATCH_REQUESTS, async (matchData) => {
      const matchId = matchData.id;
      try {
        const [report, formations] = await Promise.all([fetchMatchReport(matchId), fetchMatchFormations(matchId)]);
...
function downloadCSV() {
  ...
  allMatches.forEach((match) => {
    ...
    rows.push([
      ...
      formationInfo.homeName,
      formationInfo.awayName,
      `${homeStats.goals}-${awayStats.goals}`,
      homeStats.goals,
      awayStats.goals,
      formatFixed(homeStats.xG, 2),
      formatFixed(awayStats.xG, 2),
      homeStats.shots,
      awayStats.shots,
      ...
      homeStats.avgRating,
      awayStats.avgRating,
      formationInfo.homeFormationType,
      formationInfo.awayFormationType,
      formationInfo.homeXIOverall,
      formationInfo.awayXIOverall,
    ]);
  });
